...
import { viewGig } from "./gig.controllers.js";


const generateauthToken = (userId) => {
    return jwt.sign({ id: userId }, process.env.JWT_SECRET_KEY, { expiresIn: '60m' });
  };
...
export const registerUser = async (req, res) => {
    ...
    try {
        ...
        // Check if the image was uploaded successfully and is present in the request body. If so, add the url to the request body so it can be saved to the database
        if (req.file) {
            reqUserInfo.profileImageUrl = req.file.path;
        }
        ...
        if (await userModel.findOne({
            ...
            return res.send(`This user already exists, kindly login instead`);
        }
        ...
        await sendVerificationMail(modelUser.userName, modelUser.email, token);
        ...
        const sanitizedUser = {
            ...
            profileImageUrl: modelUser.profileImageUrl,
            role: modelUser.role
        }

        res.status(201).json({ message: "User was added successfully", data: sanitizedUser, token: authenticationToken });
...
export const deleteUser = async (req, res) => {
    const id = req.user.id; // this is the id of the user that was extracted from the token and saved in the req.user object in the authenticator middleware
    try {
...
export const switchRole = async (req, res) => {
    ...
    try {
         await userModel
        .findByIdAndUpdate(id, newUserUpdates, { new: true });
        viewGig(req, res); // Call the viewGig function to view the gigs of the user that was updated
